...
class _BoardScreenState extends State<BoardScreen> {
  ...
  Future<void> _refreshUserData() async {
    try {
      final response = await http.get(
        Uri.parse('${AppConstants.baseUrl}/user/${widget.currentUser.id}'),
      );
  ...
  Future<void> _fetchPosts({bool isRefresh = false}) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
          '${AppConstants.baseUrl}/posts?page=$_currentPage&limit=$_limit',
        ),
      );
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          "content": content,
          "is_boost":
              isBoost, // ส่งไปบอก Server (ถ้า VIP Server จะรู้เองว่าไม่ตัดเงิน)
        }),
      );
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (isBoost && !widget.currentUser.isVip)
          setState(() => widget.currentUser.coinBalance += 50);
        ...
        showDialog(
          ...
          builder:
              (ctx) => AlertDialog(
                ...
                content: Text(
                  errorData['message'] ??
                      "วันนี้คุณใช้สิทธิ์ฟรีครบ 5 ครั้งแล้ว",
                ),
                actions: [
                  TextButton(
      ...
      body: RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            bool isBoosted =
                (post['is_boost'] == 1 || post['is_boost'] == true);
