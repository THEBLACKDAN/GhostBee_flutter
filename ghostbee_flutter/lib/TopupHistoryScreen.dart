  ...
  Future<void> _fetchHistory() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("$baseUrl/payment/history/${widget.currentUser.id}"),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ...
          : historyList.isEmpty
              ? const Center(child: Text("ไม่พบประวัติการเติมเงิน")) // ใช้ const
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final date = DateFormat('dd MMM yyyy HH:mm').format(DateTime.parse(item['created_at']));
                    final double amountToDisplay = double.tryParse(item['amount'].toString()) ?? 0.0;
                    final int coinsAdded = int.tryParse(item['coins_added'].toString()) ?? 0;
                    final Color statusColor = _getStatusColor(status);

                    return Card(
                      margin: const EdgeInsets.symmetric(vertical: 8), // ใช้ const
                      elevation: 3,
                      child: ListTile(
                        ...
                        title: Text(
                          ...
                          style: const TextStyle(fontWeight: FontWeight.bold), // ใช้ const
                        ),
                        subtitle: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text("สถานะ: ${status.toUpperCase()}", style: TextStyle(color: statusColor)), // ใช้ตัวแปร
                            if (status == 'success')
                              Text("ได้รับ: $coinsAdded Coins", style: const TextStyle(fontWeight: FontWeight.w600)), // ใช้ const
                            if (item['message'] != null)
                               Text("ข้อความ: ${item['message']}", style: const TextStyle(fontSize: 12, fontStyle: FontStyle.italic)), // ใช้ const
                            Text("วันที่: $date", style: const TextStyle(fontSize: 12)), // ใช้ const
                          ],
                        ),
                        trailing: status == 'pending' 
                            ? const Icon(Icons.refresh, color: Colors.orange) // ใช้ const
                            : null,
                        onTap: () {
